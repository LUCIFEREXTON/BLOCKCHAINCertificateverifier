...
router.post('/login', (req, res) => {
    ...
    User.findOne({ username })
        .then(user => {
            if (user) {
                account.hashBlock(user.nonce, user.prehash, { username, password })
                    .then(hash => {
                        if (user.hash === hash)
                            return res.render('output', {
                                certificateNumber: 0,
...
router.post('/getcertificate', upload.single('certificate'), (req, res) => {
        ...
        .then(async(certifound) => {
            if (certifound) {
                ...
                try {
                    ...
                    const block = await certificate.createNewBlock(nonce, prehash, hash)
                    ...
                    newCerti.save()
                        .then(certi => {
                            if (certi) {
                                ...
                                return res.render('output', {
                                    certificateNumber: 0,
                                    status: 'Certificate Number generation unsuccessful created try again'
                                })
                            }
                        })
                        .catch(err => {
